...
**Pastille de glance (Lot 9) — FAITE** (`9066849` + `bc7ff0a`, choix Ludo « construire le vrai lien ») :
- `EntitySuggestion` rattache le contact né dans le chat à sa conversation (`scope='conversation'` + `scope_id`).
- `ConversationMemoryChip` : signal lecture seule au-dessus de l'input (« N contacts liés à cette conversation »), 1 action (clic → vue Mémoire), rien si hors conversation/aucun lié.
- **Bug d'intégration corrigé** : `ContactCreate`/`ContactResponse` n'avaient pas `scope`/`scope_id` → le scope était jeté à la création ET jamais renvoyé. Conséquence : le filtre « Conv. » de la Mémoire était **déjà cassé** (jamais détecté) et la pastille ne pouvait pas s'afficher. Round-trip complété + vérifié en live. Attrapé par la vérif d'intégration (invisible aux tests unitaires qui mockent le store) — illustration concrète de la valeur du « 2e regard » réel.

**Reste de la reco (post-L6)** : fusion onglet Contacts → vue CRM + sortie onglet Fichiers → vue Indexation (Lot 10), virtualisation de la liste contacts (scalabilité). Vérif visuelle navigateur de la pastille à refaire quand l'extension Chrome est dispo (les captures ont timeout cette session ; logique couverte par 7 tests + round-trip live).
