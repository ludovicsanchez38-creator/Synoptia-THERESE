...
- [~] **Phase 1 — refonte navigation** (content-swap par vues) : layout validé (zone principale, Linear/Notion), spec (`phase1-navigation-spec.md`), `navigationStore` (6 tests). **L1 FAIT + vérifié navigateur** : le CRM est une VUE plein écran de la zone principale (routeur via navigationStore) avec retour '← Chat', plus une fenêtre Tauri (tsc 0, 134 tests front). Reste L2-L5 (Email/Agenda/Tâches/Factures, même patron), L7 (pile Échap unifiée), L8 (⌘K), L9 (nettoyage windowManager).
