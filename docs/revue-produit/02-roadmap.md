...
- [~] **Phase 1 — refonte navigation** (content-swap par vues) : layout validé (zone principale, Linear/Notion), spec (`phase1-navigation-spec.md`), `navigationStore`. **FAITS** : L1 (CRM en vue, vérifié navigateur), **L2-L5 (Email/Agenda/Tâches/Factures en vues, `b365131`)**, **L9 (machinerie fenêtres-panels supprimée : windowManager + PanelWindow, `c4ed628`)**. **RESTE** :
  - **L6 — Mémoire : vue plein écran vs tiroir contextuel** → **panel UX/UI rendu** (`L6-decision-ux-panel.md`, 6 experts + 2 critiques adversariales). **Reco : vue plein écran** (certitude forte), le glance servi par un signal contextuel léger inline (pas un tiroir). Les critiques ont ouvert le code : le « débordement » vient d'un `min-w-0` manquant (pas du tiroir), le tiroir masque déjà le chat (backdrop), CRM+Mémoire partagent déjà `useContactsStore`. **En attente arbitrage Ludo.**
