# Release v0.49.0-alpha — « ce qui est annoncé se produit »

- **Date** : 27/08/2026
- **Type** : MINOR (comportements corrigés + un état vide nouveau)
- **Branche** : `feat/ux-p4-noms`, 18 commits, merge `0aa2386b`
- **Revue** : 3 passes — 2 NO-GO puis **GO**, 7 findings fermés

> Branche `feat/ux-p4-noms`, 18 commits, 129 fichiers.
...
## La revue, et ce qu'elle a coûté

Trois passes sur le diff, deux NO-GO. **Quatre des sept findings étaient des
régressions de mes propres correctifs** :

| Correctif | Régression qu'il a introduite |
|---|---|
| `activeView` nullable (P0.1) | `null` servait déjà de sentinelle « effet non initialisé » : la **première** navigation depuis l'accueil était avalée |
| Confirmation de suppression (P0bis) | la cible survivait au changement de projet : on pouvait supprimer un fichier du projet A depuis le projet B |
| Migration des couleurs (P3) | la ligne de l'heure du calendrier, rouge **par convention**, devenue « erreur » |
| Composant `Spinner` (socle P3) | `role="status"` imposé par défaut : deux annonces pour un seul événement |

Et un cinquième cas, plus instructif : le correctif de `cle_ia` semblait fermé
et ne l'était **qu'au tiers** — deux autres `try/except` subsistaient dans la
même fonction. La revue l'a trouvé en *exécutant* le cas plutôt qu'en relisant
le code. « J'ai retiré le `try/except` » n'est pas « la fonction ne masque plus
ses échecs » : il fallait compter.

## Gates

| Gate | Résultat |
|---|---|
| pytest (hors e2e) | 2326 verts, 0 échec |
| vitest | 920 verts, 151 fichiers |
| mypy fresh | 1001 (identique à `main`, baseline CI 1004) |
| ruff / tsc / eslint | propres (eslint 27/27) |

Chaque correctif a été saboté puis restauré par remplacement inverse.

## Recette visuelle

Menée dans l'application réelle (dev server + backend sur une base sandbox
dédiée, jamais les données de production), avec des mesures au
`getComputedStyle` plutôt qu'à l'œil. Elle a trouvé deux choses qu'aucun grep
n'avait montrées : les cinq verbes de l'établi étaient en 12 px — la taille de
« Connecté » et des mentions de bas de page — et sept boutons de passage
étaient encore en « Ouvrir X ».


## Chronologie (27/08/2026)

| Heure | Étape |
|---|---|
| ~11h | Constat de Ludo, inventaire des surfaces, deux hypothèses écartées par le code |
| ~12h | Plan challengé AVANT écriture : NO-GO, 6 objections, 3 contre-vérifiées |
| 12h-15h | Exécution : noms, navigation, Échap, spinners, couleurs, panne vs vide, cartes |
| ~14h | Recette visuelle dans l'app (dev server + base sandbox) |
| 15h-16h | 3 passes de revue sur le diff : NO-GO, NO-GO, **GO** |
| 15h12 | Merge, bump 0.48.2 -> 0.49.0, tag poussé |
| 15h38 | Build 3 OS vert du premier coup (macOS 10 min, Linux 17 min, Windows 22 min) |
| 15h40 | Release publiée, landing + `latest.json` (7 plateformes signées) live |
| 15h45 | Discord posté, app installée en local, Katia et Zézette à jour |
