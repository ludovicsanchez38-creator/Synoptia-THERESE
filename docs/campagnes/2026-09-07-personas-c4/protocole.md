# Protocole : campagne « chaque écran, chaque persona », THÉRÈSE 0.67.0 + lot repair/c4-lot1 (cycle 4)

> Campagne du cycle 4, 07/09/2026, même méthode que le cycle 3. Demande de Ludo, 05/09/2026 : « ce qui permet le plus de voir les bugs, c'est les
...
- Backend jetable : `http://127.0.0.1:17393`, `THERESE_DATA_DIR=/tmp/therese-demo-c4`
  (base VIERGE au départ pour Sophie, qui fait la mise en route ; ensuite les données que
  les personas y déposent eux-mêmes, dans l'ordre Sophie, Karim, Jean, Nadia). Jeton dans `/tmp/therese-demo-c4/.session_token`,
...
- Instrument : Playwright (serveur MCP `plugin_playwright_playwright`), document toujours
  ...
  à 1024×768 et en thème sombre. Le frontend vient du worktree de réparation
  (`Synoptia-THERESE-repair`, branche `repair/c4-lot1`), donc avec les correctifs du cycle 4.
...
1. capture plein écran : `browser_take_screenshot` avec `filename` =
   `docs/campagnes/2026-09-07-personas-c4/captures/<persona>/<NN>-<capacite>-<etape>.png`
...
Fichier : `docs/campagnes/2026-09-07-personas-c4/rapports/<persona>.md`, en français.
