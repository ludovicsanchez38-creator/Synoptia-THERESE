...
Version 5, 11/09/2026 09:20, après la revue de la v4 (VERDICT : GO, 5 points repris, 0 non repris) ; journal `.cartography-work/reviews/grok-da-lot6-projets-design-v4.log`. Précédente : v4, après la revue de la v3 (6 points repris, 0 non repris), journal `.cartography-work/reviews/grok-da-lot6-projets-design-v3.log`. Précédent : lot 3 (Tiroir), sur `main` ; cadence : une
...
1. **Deux vues restent deux vues.** `tasks` et `projects` ne fusionnent pas
   (P-076). Le libellé visible « Tâches » / « Projets » reste un `<p>`
   (B-241), pas un `h1`.
2. La création d'une tâche reste `TaskForm` qui **remplace** le corps (pas
   la saisie rapide au-dessus des colonnes, P-077). L'état `nouvelle` de la
...
| Compteur | `{n} tâche` / `{n} tâches` (B-210, `n > 1`) | **exactement** ces deux chaînes ; pas « ouvertes », pas de retard, pas de projets (P-078) |
...
Pas de ligne « Projet : tous · Échéance : toutes · Tri : échéance » (P-078).
...
| Échéance | `toLocaleDateString` jour + mois court, `text-xs` | inchangée (pas « Aujourd'hui » ni `JJ/MM`, P-078) ; `text-xs font-medium text-text-muted` |
...
| Projet sur la carte | absent | reste absent (P-078) |
| Commandes | icônes 12 px, `title` seul, overlay `absolute top-2 right-2` (`TaskKanban.tsx:401-406`) ; `showActions && !isOverlay` démonte | `Button variant="ghost" size="icon"` `aria-label` = les `title` actuels (« Marquer en cours », « Marquer terminé », « Rouvrir ») ; **rangée dans le flux** de la carte, après les tags : `flex items-center gap-1 mt-2`, `onClick` stoppe la propagation ; **pas** `absolute top-2 right-2` (trois `h-9 w-9` recouvriraient titre et barre) ; **toujours montée** si `!isOverlay` (hauteur de carte stable : aujourd'hui l'`absolute` ne pousse pas le flux) ; hors survol/focus (`showActions === false`) : `aria-hidden={!showActions}` **et** `pointer-events-none` en plus de la classe `invisible` (revue v4 : `vitest.config.ts:7` `environment: 'jsdom'` et `src/test/setup.ts` n'importe aucun CSS, `invisible` n'y masque rien ; seul `aria-hidden` retire de l'arbre a11y, donc rend `queryByRole` nul) (Tailwind `visibility: hidden`, **pas** `opacity-0` — `focusVisibleSurActions.test.ts` interdit le motif `opacity-0 group-hover` autour d'un bouton ; **pas** `hidden` = `display:none`, qui collapserait la hauteur). `TaskKanban.clavier.test.tsx:62` exige `queryByRole('button', { name: 'Marquer terminé' })` = null avant focus : `invisible` retire de l'arbre a11y, `opacity-0` non. Overlay : rangée non montée. Révélation sur le conteneur `useSortable` (B-209 : le focus atterrit là, pas sur la carte) : `onFocus={() => setFocusDansLaCarte(true)}` et `onBlur` avec la garde `if (!event.currentTarget.contains(event.relatedTarget)) setFocusDansLaCarte(false)` (`TaskKanban.tsx:272-278`). `showActions` = survol **ou** focus-within (la garde) : sans `contains`, un `onBlur` naïf au Tab vers « Marquer terminé » pose `showActions === false` et `invisible` cache le bouton focalisé. `TaskKanban.clavier.test.tsx:79-86` ne blur que vers `document.body` : ajouter un cas blur vers le bouton (`relatedTarget` = le bouton « Marquer terminé »), `queryByRole` non null |
| « Ajouter une tâche » en pied de colonne | absent | reste absent (P-077) |
...
colonne de la coque, 56 rem, suffit ; 72 rem de la maquette est P-076).
Dialogue de suppression : `role="dialog"` `aria-modal="true"`
`aria-labelledby="delete-project-title"` conservés, piège de focus et
`pushEscapeHandler` inchangés ; overlay `bg-text/35` (plus de `bg-black/50` ; revue v4 : la coque DA pose `bg-text/35`, `VoilePanneau.tsx:18`) ;
...
| 2 | `error` | `Alerte` **frère au-dessus** de la cascade `loading` / formulaire / vue, jamais une branche exclusive (revue v4 : aujourd'hui le bandeau est un frère et le corps reste, `TasksPanel.tsx:268-285` ; une erreur de rafraîchissement cacherait sinon `TaskForm`) ; pas de `data-testid` aujourd'hui, on n'en invente pas, `icone` `AlertCircle` 18 px, `children` = « Impossible de charger les tâches » ou « Impossible de rafraîchir les tâches : la liste affichée peut être périmée. » ; **pas** d'`action` Réessayer (il n'y en a pas) ; le corps en cache s'affiche encore dessous (B-532) |
...
- Un seul écran « Tâches » + grille « Projets » dessous, colonne 72 rem : aujourd'hui deux destinations (`tasks`, `projects`). Fonctionnalité : **P-076** (numéro à confirmer par l'orchestrateur).
- Saisie rapide (titre, Entrée, Échap, projet courant) et « Ajouter une tâche » en pied de colonne ; « trois secondes depuis n'importe quel écran ». Fonctionnalité : **P-077**.
- Compteurs « N tâches ouvertes · M en retard · K projets actifs », résumé de filtres, tri par échéance, filtre d'échéance, nom du projet et pastille sur la carte, « En retard de N jours » / « Aujourd'hui » / `JJ/MM`. Fonctionnalité : **P-078**.
- Cartes projet en grille (barre de progression, devis/facture, séance, pastille de domaine) à la place du kanban par statut. Fonctionnalité : **P-079**.
...
Aucun des 5 points de `.cartography-work/reviews/grok-da-lot6-projets-design-v4.log` (VERDICT : GO) n'est écarté : chacun est fondé (preuve relue dans la maquette, `globals.css`, `VoilePanneau.tsx`, `TasksPanel.tsx` et `vitest.config.ts`) et repris dans les sections ci-dessus.
