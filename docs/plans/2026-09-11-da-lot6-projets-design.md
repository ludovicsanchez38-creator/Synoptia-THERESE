...
6. `Ligne` n'est pas montée : une case à cocher et une corbeille dans la
   ...
   identique (`grid-cols-[2rem_1fr_auto]`). `ProjectModal` est une couche
   sans maquette : hors lot.
...
| Carte | `bg-surface-elevated/60`, badge priorité `text-xs`, `whileHover scale` | `bg-surface border border-border rounded-sm p-3` ; plus de `scale` ; barre `w-1 h-[1.1rem] rounded-sm shrink-0` + `aria-label="Priorité urgente / haute / moyenne / basse"` : urgent et high `bg-error`, medium `bg-warning`, low `bg-border` ; plus le mot « Urgent » / « Haute » / « Moyenne » / « Basse » |
| Retard | « En retard » `text-xs` + `AlertCircle` 12 px | `Etiquette ton="erreur"` « En retard » (pas « de N jours », P-073) |
| Titre | `h4 text-sm` | `font-semibold text-sm text-text` ; `done` : `line-through text-text-muted` (les deux vues) |
| Description | `text-sm line-clamp-2` | inchangée (corps, B-134) |
...
| Commandes | icônes 12 px, `title` seul | `Button variant="ghost" size="icon"` `aria-label` = les `title` actuels (« Marquer en cours », « Marquer terminé », « Rouvrir ») ; révélation focus + survol inchangée (B-209) |
...
`grid grid-cols-[2rem_1fr_auto] gap-3 items-start px-4 py-3 border-t border-border hover:bg-surface-2 relative` ; plus de `scale`, plus d'`opacity-60` (le titre `done` se barre). Case à cocher : `Button variant="ghost" size="icon"` dans la colonne 2 rem, `relative z-10`, `aria-label` actuels conservés (`Marquer la tâche ${title} terminée` / `Rouvrir la tâche ${title}`), icônes `CheckCircle2` / `Circle` 18 px. Titre : `<button type="button" aria-label={`Ouvrir la tâche ${title}`}>` conservé (D105), `className="font-semibold text-left text-text"` + `line-through text-text-muted` si `done`. Description `text-sm text-text-muted line-clamp-1` inchangée. `droite` (`flex items-center gap-2 relative z-10`) : `Etiquette ton="erreur"` « En retard » si dû, échéance `text-xs font-medium text-text-muted` (liste : jour + mois court + année, format actuel), tags `Etiquette ton="neutre"`, corbeille `Button ghost icon` `aria-label={`Supprimer la tâche ${title}`}`. L'icône `Clock` 18 px `text-info` reste si `in_progress` (pas de détail
« · en cours » inventé, P-073). Confirmation en ligne : `div` (pas `Alerte` : ce n'est pas une erreur, aujourd'hui sans `role="alert"`), textes exacts « Supprimer « … » ? », « Cette action est irréversible. », `Button ghost md` « Conserver la tâche », `Button danger md` « Supprimer définitivement » ; `onClick` stoppe la propagation, Échap via `pushEscapeHandler` inchangé.
...
Remplace toujours le corps. En-tête : `Button ghost icon` `aria-label="Retour"` (le chevron n'avait pas de nom) + `<h3>` « Nouvelle tâche » / « Modifier la tâche » ; enregistrer `Button primary md`, libellé « Enregistrer » ou, si `saving`, `Spinner taille="bouton"` + « Enregistrement... » (points de suspension conservés). Champs dans l'ordre actuel, `FormField` + primitive, `htmlFor` conservés :

| `htmlFor` | Label | Primitive | Notes |
|---|---|---|---|
| `taskform-titre` | Titre | `Input` | `required` `aria-required="true"` (B-621) ; `placeholder="Titre de la tâche"` ; `error` si titre vide à l'enregistrement |
| `taskform-description` | Description | `Textarea` | `rows={4}`, placeholder actuel |
| `taskform-statut` | Statut | `Select` | À faire / En cours / Terminé / Annulé, mêmes `value` |
| `taskform-priorite` | Priorité | `Select` | Basse / Moyenne / Haute / Urgent, mêmes `value` |
...
même `handleNew`. Wrapper du kanban : `Carte`. `max-w-[760px]` retiré (la
...
Quatre groupes, libellés en casse de phrase (P-046) : Actif, En attente, Terminé, Annulé. Tête : `Etiquette` succes / attention / info / erreur + compte `text-sm tabular-nums text-text-muted` ; plus de `uppercase`, plus de `text-agent-*`. Vide de groupe : « Glisser ici ». Vide total : `EtatVide` titre « Aucun projet », sans action (le geste est en tête). Carte : `bg-surface border border-border rounded-sm`, plus de `scale` ; nom `text-sm font-semibold` ; description `text-sm text-text-muted` (plus `text-xs` sur ce texte, le plancher) ; budget inchangé ; supprimer `Button ghost icon` `aria-label={`Supprimer ${name}`}` conservé. Pas de barre de progression, pas de pièce, pas de séance (P-074).
...
|---|---|---|
| 1 | `loading` et pas de cache | trois rangées `aria-hidden` façon carte : puce `Squelette largeur="w-8" classeBarre="h-8 rounded-sm"`, puis `w-[60%]` et `w-[40%]` ; **sans** nouvelle chaîne (le `RefreshCw` d'aujourd'hui est muet) |
...
| Priorité | Condition | Cible |
|---|---|---|
| 1 | `loading` | trois rangées `Squelette` `aria-hidden` + « Chargement des projets… » en `role="status"` (le `Spinner` seul disparaît ; le texte, lui, existe déjà) |
...
| 3 | `projects.length === 0` | `EtatVide` titre « Aucun projet », sans action |
| 4 | sinon | `Carte` + `ProjectsKanban` ; si troncature, le `role="alert"` warning est **au-dessus**, zéro Réessayer |
...
`TaskKanban.da.test.tsx` : 1) trois `Etiquette` + comptes ; 2) barre `aria-label="Priorité moyenne"` (plus le mot « Moyenne ») ; 3) « En retard » porte `data-etiquette` ; 4) `task-item` présent, absent de l'overlay ; 5) « Marquer terminé » nommé, révélé au focus.

`TaskList.da.test.tsx` : 1) grille `2rem 1fr auto` ; 2) D105 / D106 noms conservés ; 3) `EtatVide` + « Créer une tâche » `md` ; 4) confirmation : pas `role="alert"` sur le bandeau.

`TaskForm.da.test.tsx` : 1) titre `required` + `aria-invalid` quand l'erreur maquette est posée ; 2) chaîne exacte « Ajoute un titre : c'est la seule chose obligatoire. » ; 3) ids `taskform-*` conservés ; 4) Retour nommé.

`ProjectsPanel.da.test.tsx` : 1) « 199 projets » sans alerte, 200 → `role="alert"` /Liste incomplète/ et pas « 200 projets » ; 2) un « Réessayer » sur erreur, zéro sinon ; 3) « Nouveau projet » `md` ; 4) pas de `bg-black/`.

`ProjectsKanban.da.test.tsx` : 1) « Actif » sans `uppercase` ; 2) `Etiquette` sur les quatre têtes ; 3) `EtatVide` « Aucun projet ».

`aucuneCouleurEnDur.test.ts` étendu aux six fichiers.

À aligner, forme seulement : `tailleDuCorps.test.tsx` (`getByText('Moyenne')` + `text-xs` → `getByLabelText('Priorité moyenne')` ; la description reste `text-sm` ; l'échéance, présente dans le jeu, porte `text-xs`) ; `TasksPanel.nomsAccessibles.test.tsx` (les bascules n'ont plus de `title` : leur nom est « Colonnes » / « Liste », le cas « nom du seul title » reste vert). Aucune assertion de comportement n'est retirée. Inchangés : `TasksPanel.orthographe.test.tsx`, `accentAFaire.test.tsx`, `TaskKanban.test.tsx` / `.clavier.test.tsx`, `TaskList.cycle6` / `.etatVide` / `.nomsAccessibles` / `.suppression.cycle6`, `TaskForm.titreRequis` / `.cycle6`, `ProjectsKanban.test.tsx` / `.casse.test.tsx`, `ProjectsPanel.troncature.test.tsx`.
