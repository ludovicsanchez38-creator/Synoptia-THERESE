#!/usr/bin/env python3
"""
Import CRM data directly to database
"""
...
def import_contacts_direct():
    """Import contacts directly to database"""
    ...
    with Session(engine) as session:
        for contact_data in crm_data:
            ...
            # Préparer les tags en JSON
            tags_list = [tag.strip() for tag in contact_data.get("Tags", "").split(",") if tag.strip()]
            ...
            try:
                ...
                session.commit()
                created += 1
                print(f"✓ {full_name} créé (stage: {contact.stage}, score: {contact.score})")
            except Exception as e:
                print(f"✗ {full_name} erreur: {e}")
                session.rollback()
                skipped += 1
