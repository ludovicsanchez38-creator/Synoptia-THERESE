...
async def _do_stream_response(
    ...
    if tools:
        ...
        if "invoice_totals" in tool_names:
            capabilities += "- **invoice_totals** : Totalise ce qu'il RESTE A ENCAISSER (factures emises non payees, part en retard, avoirs deduits). Utilise-le pour toute question de tresorerie SANS nom ni reference : « combien il me reste a encaisser », « quelles factures ne sont pas payees », « combien on me doit ». N'utilise PAS search_invoices pour ca : il cherche UN document, il ne totalise rien. `encours_ttc` vaut null des qu'il y a plusieurs devises OU que le net n'est plus positif : il n'y a alors AUCUN total global, cite `encours_par_devise` montant par montant sans en fabriquer un.\n"
