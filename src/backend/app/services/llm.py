...
- Le STOCKAGE des données métier est local, dans le dossier de données de l'application (par défaut `~/.therese/`, déplaçable) :
  - **base SQLite : chiffrée au repos, SQLCipher (AES-256)** — sauf si l'application a été démarrée avec l'échappatoire de débogage prévue quand le trousseau est indisponible. La clé est dérivée d'une clé maîtresse gardée dans le trousseau du système quand il est disponible, avec une copie de secours dans un fichier local. Sur macOS et Linux, ce fichier est restreint au seul compte utilisateur (0600) ; sur Windows, cette restriction n'existe pas telle quelle et le fichier suit les droits du dossier qui le contient.
...
- Distingue TOUJOURS deux couches : le STOCKAGE (local, ci-dessus) et le TRAITEMENT de la conversation. Le traitement peut sortir vers le fournisseur du modèle SI un modèle cloud est actif (ex. Mistral) ; avec un modèle local (Ollama), le contenu de la conversation ne part pas chez un fournisseur de modèle, SAUF avec un modèle Ollama Cloud (nom terminé par « :cloud » ou « -cloud »), traité sur les serveurs d'Ollama.
