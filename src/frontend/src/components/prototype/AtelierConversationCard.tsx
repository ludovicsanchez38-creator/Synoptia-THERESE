    ...
    <section aria-labelledby="atelier-history-title" className="overflow-hidden rounded-[16px] border border-border bg-surface shadow-[0_12px_28px_-22px_rgba(16,28,54,0.45)]" data-testid="atelier-history-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onNewMission} className="inline-flex items-center gap-1.5 rounded-[8px] bg-accent-fill px-2.5 py-1.5 text-xs font-semibold text-accent-ink"><Plus className="h-3.5 w-3.5" />Nouvelle mission</button>
  ...
  return (
    <section className="rounded-[13px] border border-border bg-surface p-4" data-testid="atelier-preflight">
      ...
      <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2">
        <div className="rounded-[9px] bg-surface-2 p-3"><dt className="text-xs uppercase tracking-wide text-text-muted">Dépôt autorisé</dt><dd className="mt-1 break-all font-semibold text-text">{status.repo_path || config.source_path || 'Non configuré'}</dd></div>
        <div className="rounded-[9px] bg-surface-2 p-3"><dt className="text-xs uppercase tracking-wide text-text-muted">État Git</dt><dd className="mt-1 font-semibold text-text">{status.current_branch || 'Branche inconnue'} · {status.working_tree_clean === true ? 'dépôt propre' : status.working_tree_clean === false ? 'changements non enregistrés' : 'état inconnu'}</dd></div>
        <div className="rounded-[9px] bg-surface-2 p-3"><dt className="text-xs uppercase tracking-wide text-text-muted">Katia</dt><dd className="mt-1 font-semibold text-text">{config.katia_model}</dd></div>
        <div className="rounded-[9px] bg-surface-2 p-3"><dt className="text-xs uppercase tracking-wide text-text-muted">Zézette</dt><dd className="mt-1 font-semibold text-text">{config.zezette_model}</dd></div>
      </dl>
    </section>
  );
}
...
function TaskDetail({
  ...
  return (
    <div className="space-y-4" data-testid="atelier-task-detail">
      ...
      {task.diff_summary && <section className="rounded-[13px] border border-border bg-surface p-4"><h4 className="text-xs font-bold text-text">Résumé Git enregistré</h4><pre className="mt-2 whitespace-pre-wrap text-xs leading-5 text-text-muted">{task.diff_summary}</pre></section>}
