    ...
    <section aria-labelledby="atelier-history-title" className="overflow-hidden rounded-[16px] border border-border bg-surface shadow-[0_12px_28px_-22px_rgba(16,28,54,0.45)]" data-testid="atelier-history-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onNewMission} className="inline-flex items-center gap-1.5 rounded-[8px] bg-text px-2.5 py-1.5 text-xs font-semibold text-white"><Plus className="h-3.5 w-3.5" />Nouvelle mission</button>
          <button type="button" onClick={onOpenClassic} className="rounded-[8px] border border-border px-2.5 py-1.5 text-xs font-semibold text-text">Atelier complet</button>
        </div>
      </div>
...
export function AtelierWorkspaceCanvas({
  ...
  return (
    <div className="flex h-full flex-col"><div className="border-b border-border px-5 py-4 pr-16"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-text-muted"><Code2 className="h-3.5 w-3.5" />Atelier réel</div><h2 className="mt-2 text-xl font-bold tracking-[-0.02em] text-text">{target === 'new-mission' || target === 'current' ? 'Mission de changement' : 'Mission enregistrée'}</h2><p className="mt-1 text-sm text-text-muted">Cadrage, worktree isolé, progression réelle, diff Git et validation séparée.</p></div>
      <div className="min-h-0 flex-1 overflow-y-auto p-5">{resource.status === 'loading' ? <StateShell><div className="flex items-center gap-2 text-sm text-text-muted"><Spinner taille="bouton" />Chargement de l’Atelier…</div></StateShell> : resource.status === 'error' ? <StateShell><div className="text-center"><AlertCircle className="mx-auto h-5 w-5 text-warning" /><p className="mt-2 text-sm font-semibold text-text">{resource.error}</p><button type="button" onClick={onRetry} className="mt-4 rounded-[9px] bg-text px-3 py-2 text-xs font-semibold text-white">Réessayer</button></div></StateShell> : showRun ? <AtelierRunView run={run} taskResource={taskResource} diffResource={diffResource} actionPending={actionPending} onCancel={onCancel} onReset={onReset} onMutate={onMutate} /> : target === 'new-mission' || target === 'current' ? <NewMissionForm workspace={resource.data} run={run} onStart={onStart} /> : !taskResource || taskResource.status === 'loading' ? <StateShell><div className="flex items-center gap-2 text-sm text-text-muted"><Spinner taille="bouton" />Chargement de la mission…</div></StateShell> : taskResource.status === 'error' ? <StateShell><div className="text-center"><AlertCircle className="mx-auto h-5 w-5 text-warning" /><p className="mt-2 text-sm font-semibold text-text">{taskResource.error}</p><button type="button" onClick={onRetryTask} className="mt-4 rounded-[9px] bg-text px-3 py-2 text-xs font-semibold text-white">Réessayer</button></div></StateShell> : <TaskDetail task={taskResource.data} diffResource={diffResource} actionPending={actionPending} onMutate={onMutate} />}</div>
      <div className="border-t border-border bg-surface p-4"><button type="button" onClick={onOpenClassic} className="flex w-full items-center justify-center gap-2 rounded-[10px] bg-text px-4 py-3 text-sm font-semibold text-white"><ExternalLink className="h-4 w-4" />Gérer les améliorations</button></div></div>
  );
}
