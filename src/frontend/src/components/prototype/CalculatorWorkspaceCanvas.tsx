      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
        ...
        <section id={`calculator-panel-${active}`} role="tabpanel" aria-labelledby={`calculator-tab-${active}`} className="mt-4 rounded-[13px] border border-border bg-surface p-4">
          ...
          <fieldset disabled={pending} className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            {active === 'npv' && <><InputField label="Investissement initial" value={values.initialInvestment} onChange={(value) => update('initialInvestment', value)} suffix="€" min={0} /><InputField label="Taux d’actualisation par période" value={values.discountRate} onChange={(value) => update('discountRate', value)} suffix="%" min={0} max={100} /><label className="text-xs font-semibold text-text sm:col-span-2">Flux de trésorerie par période<textarea aria-label="Flux de trésorerie par période" value={values.cashFlows} onChange={(event) => update('cashFlows', event.target.value)} rows={3} placeholder="30000 ; 40000 ; 50000" className="mt-1.5 w-full resize-y rounded-[9px] border border-border px-3 py-2 text-sm font-normal outline-none focus:border-[#22D3EE]" /><span className="mt-1 block text-xs font-normal text-text-muted">Sépare les périodes par un point-virgule ou un retour à la ligne. Le taux et les flux doivent utiliser la même période, par exemple une année.</span></label></>}
