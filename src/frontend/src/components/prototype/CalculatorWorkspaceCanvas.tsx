...
function ResultCard({ result }: { result: CalculatorResult }) {
  ...
  return (
    <section className="rounded-md border border-accent-cyan/30 bg-accent-tint p-4" data-testid="calculator-result" role="status" aria-live="polite">
...
export function CalculatorWorkspaceCanvas({ onClose }: { onClose: () => void }) {
  ...
  return (
    <aside ref={dialogRef} role="region" aria-labelledby="calculator-workspace-title" tabIndex={-1} className="absolute inset-y-0 right-0 z-20 flex h-full w-full max-w-[620px] flex-col border-l border-border bg-surface-2 shadow-[-18px_0_45px_rgba(16,28,54,0.12)] sm:w-[calc(100%-48px)] xl:relative xl:w-[43%] xl:min-w-[440px] xl:shadow-none" data-testid="calculator-workspace-canvas">
      <button type="button" onClick={onClose} aria-label="Fermer les calculateurs" className="absolute right-3 top-3 z-30 grid h-11 w-11 place-items-center rounded-md border border-border bg-surface text-text-muted shadow-sm hover:text-text"><PanelRightClose className="h-4 w-4" /></button>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
        ...
        <section id={`calculator-panel-${active}`} role="tabpanel" aria-labelledby={`calculator-tab-${active}`} className="mt-4 rounded-md border border-border bg-surface p-4">
          ...
          {error && <div id="calculator-form-error" role="alert" className="mt-3 rounded-md border border-error/40 bg-[var(--color-error-tint)] px-3 py-2 text-sm leading-5 text-error">{error}</div>}
          <button type="button" disabled={pending} onClick={() => void calculate()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-md bg-accent-fill px-4 py-3 text-sm font-semibold text-accent-ink disabled:opacity-60">{pending && <Spinner taille="bouton" />}Calculer avec le moteur local</button>
        ...
        <div className="mt-4 flex items-start gap-2 rounded-md border border-accent-cyan/30 bg-accent-tint p-3 text-xs leading-5 text-accent"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />Le résultat vient de la formule affichée et de l’API locale. Il n’est ni mémorisé ni transmis à un modèle IA.</div>
      </div>
    </aside>
  );
}
