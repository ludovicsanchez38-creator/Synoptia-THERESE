...
import { Alerte, Button, Carte, Input, Select, Textarea } from '../ui';
...
function ResultCard({ result }: { result: CalculatorResult }) {
  ...
  return (
    <Carte as="section" className="border-accent-cyan/30 bg-accent-tint p-4" data-testid="calculator-result" role="status" aria-live="polite">
...
export function CalculatorWorkspaceCanvas({ onClose }: { onClose: () => void }) {
  ...
  return (
    <aside ref={dialogRef} role="region" aria-labelledby="calculator-workspace-title" tabIndex={-1} className="absolute inset-y-0 right-0 z-20 flex h-full w-full max-w-[620px] flex-col border-l border-border bg-surface-2 shadow-lg sm:w-[calc(100%-48px)] xl:relative xl:w-[43%] xl:min-w-[440px] xl:shadow-none" data-testid="calculator-workspace-canvas">
      <Button type="button" variant="secondary" size="icon" onClick={onClose} aria-label="Fermer les calculateurs" className="absolute right-3 top-3 z-30"><PanelRightClose className="h-4 w-4" /></Button>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
        <div role="tablist" aria-label="Type de calcul" className="grid grid-cols-5 gap-1 rounded-md border border-border bg-surface p-1">
          {CALCULATORS.map((calculator) => (
            <button key={calculator.id} id={`calculator-tab-${calculator.id}`} role="tab" aria-selected={active === calculator.id} aria-controls={`calculator-panel-${calculator.id}`} tabIndex={active === calculator.id ? 0 : -1} type="button" disabled={pending} onKeyDown={(event) => handleRovingFocus(event, '[role="tab"]', 'horizontal')} onClick={() => select(calculator.id)} className={`min-h-9 rounded-sm px-2 py-2 text-sm font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${active === calculator.id ? 'bg-accent-fill text-accent-ink' : 'text-text-muted hover:bg-bg'}`}>{calculator.label}</button>
          ))}
        </div>

        <Carte as="section" id={`calculator-panel-${active}`} role="tabpanel" aria-labelledby={`calculator-tab-${active}`} className="mt-4 p-4">
          ...
          {error && <Alerte id="calculator-form-error" className="mt-3" titre="Calcul impossible">{error}</Alerte>}
          <Button type="button" size="lg" disabled={pending} onClick={() => void calculate()} className="mt-4 w-full">{pending && <Spinner taille="bouton" />}Calculer avec le moteur local</Button>
        ...
        <div className="mt-4 flex items-start gap-2 rounded-md border border-accent-cyan/30 bg-accent-tint p-3 text-sm leading-5 text-accent"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />Le résultat vient de la formule affichée et de l’API locale. Il n’est ni mémorisé ni transmis à un modèle IA.</div>
      </div>
    </aside>
  );
}
