...
export function ImagesWorkspaceCanvas({ onClose }: { onClose: () => void }) {
  ...
  async function confirmGeneration() {
    ...
    setErrorContext(null);
    try {
      if (!hasCloudConsent('images', snapshot.request.provider)) {
        grantCloudConsent('images', snapshot.request.provider, [
          'description du visuel', 'format', 'qualité',
        ]);
      }
  ...
  return (
    <aside ref={dialogRef} role="region" aria-labelledby="images-workspace-title" tabIndex={-1} className="absolute inset-y-0 right-0 z-20 flex h-full w-full flex-col border-l border-border bg-surface-2 shadow-[-18px_0_45px_rgba(16,28,54,0.12)] sm:w-[calc(100%-48px)] xl:relative xl:w-[62%] xl:min-w-[720px] xl:shadow-none" data-testid="images-workspace-canvas">
      <header className="flex shrink-0 items-start gap-3 border-b border-border bg-surface px-5 py-4 pr-16">
        <span className="grid h-9 w-9 place-items-center rounded-md border border-text bg-domaine-factures-tint text-domaine-factures shadow-[var(--shadow-card)]"><ImageIcon className="h-4 w-4" /></span>
        <div>
        ...
        <button type="button" onClick={onClose} aria-label="Fermer le studio Images" className="absolute right-3 top-3 grid h-11 w-11 place-items-center rounded-md border border-border bg-surface text-text-muted"><PanelRightClose className="h-4 w-4" /></button>
      </header>
          ...
          <fieldset className="mt-4">
            ...
            <div className="mt-2 grid gap-2">
              {PROVIDERS.map((item) => {
                ...
                return <label key={item.id} className={`flex items-center gap-2 rounded-md border px-3 py-2 text-xs ${provider === item.id ? 'border-text bg-surface text-text' : 'border-border text-text-muted'} ${available ? '' : 'opacity-50'}`}><input id={`image-provider-${item.id}`} type="radio" name="image-provider" value={item.id} checked={provider === item.id} disabled={!available} aria-invalid={errorField === 'provider' && provider === item.id} aria-describedby={errorField === 'provider' && provider === item.id ? 'image-generation-error' : undefined} onChange={() => { setProvider(item.id); setErrorField(null); setError(null); }} /><span className="font-semibold">{item.label}</span><span className="ml-auto text-xs">{available ? 'disponible' : 'non configuré'}</span></label>;
              })}
            </div>
          </fieldset>

          <div className="mt-4 grid grid-cols-2 gap-3">
            <label className="text-xs font-semibold text-text">Format<select aria-label="Format de l’image" value={size} onChange={(event) => setSize(event.target.value as typeof size)} className="mt-2 w-full rounded-md border border-border bg-surface px-3 py-2 font-normal"><option value="1024x1024">Carré</option><option value="1536x1024">Paysage</option><option value="1024x1536">Portrait</option></select></label>
            <label className="text-xs font-semibold text-text">Qualité<select aria-label="Qualité de l’image" value={quality} onChange={(event) => setQuality(event.target.value as typeof quality)} className="mt-2 w-full rounded-md border border-border bg-surface px-3 py-2 font-normal"><option value="low">Économique</option><option value="medium">Standard</option><option value="high">Haute</option></select></label>
          </div>

          <div className="mt-4 rounded-md border border-accent-cyan/30 bg-accent-tint p-3 text-xs leading-5 text-accent"><ShieldCheck className="mr-1 inline h-4 w-4" />La demande sera transmise au moteur choisi. Rien ne part avant confirmation.</div>
          </fieldset>
          {error && <div id="image-generation-error" role="alert" className="mt-3 rounded-md border border-error/40 bg-[var(--color-error-tint)] p-3 text-sm text-error"><p><AlertCircle className="mr-1 inline h-4 w-4" />{error}</p>{errorContext && <button type="button" onClick={() => errorContext === 'load' ? void refresh() : requestGeneration()} className="mt-2 rounded-md border border-error px-3 py-2 font-semibold">Réessayer</button>}</div>}
          {confirmationSnapshot ? <div ref={confirmationRef} className="mt-4 rounded-md border border-warning/40 bg-[var(--color-warning-tint)] p-3 text-sm text-warning" data-testid="image-generation-confirmation"><strong>Confirmer la génération avec {confirmationSnapshot.providerLabel} ?</strong><p className="mt-1 font-semibold">Prompt : {confirmationSnapshot.request.prompt}</p><p>Format {confirmationSnapshot.request.size}, qualité {confirmationSnapshot.request.quality}. Cette action peut consommer un crédit du fournisseur.</p>{!hasCloudConsent('images', confirmationSnapshot.request.provider) && <p className="mt-1">En confirmant ce premier usage cloud, tu consens à transmettre ces données à {confirmationSnapshot.providerLabel}.</p>}<div className="mt-3 flex justify-end gap-2"><button type="button" onClick={() => setConfirmationSnapshot(null)} className="rounded-sm border border-border bg-surface px-3 py-2 font-semibold">Annuler</button><button type="button" onClick={() => void confirmGeneration()} disabled={pending} className="rounded-sm bg-accent-fill px-3 py-2 font-semibold text-accent-ink">Confirmer et générer</button></div></div> : <button type="button" onClick={requestGeneration} disabled={pending || loading} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md bg-accent-fill px-4 py-3 text-sm font-semibold text-accent-ink disabled:opacity-50">{pending ? <Spinner taille="bouton" /> : <Sparkles className="h-4 w-4" />}{pending ? 'Génération en cours…' : 'Préparer la génération'}</button>}
        </section>
