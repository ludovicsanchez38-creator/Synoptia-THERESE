  ...
  return (
    <aside className="absolute inset-y-0 right-0 z-20 flex h-full w-full flex-col border-l border-border bg-surface-2 shadow-[-18px_0_45px_rgba(16,28,54,0.12)] xl:w-[62%] xl:min-w-[720px]" data-testid="images-workspace-canvas">
      <header className="flex shrink-0 items-start gap-3 border-b border-border bg-surface px-5 py-4 pr-16">
        <span className="grid h-9 w-9 place-items-center rounded-[10px] border border-text bg-[var(--k3bg)] text-[var(--k3)] shadow-[2px_2px_0_#101C36]"><ImageIcon className="h-4 w-4" /></span>
        <div>
          ...
          <fieldset className="mt-4">
            ...
            <div className="mt-2 grid gap-2">
              {PROVIDERS.map((item) => {
                ...
                return <label key={item.id} className={`flex items-center gap-2 rounded-[9px] border px-3 py-2 text-xs ${provider === item.id ? 'border-text bg-surface text-text' : 'border-border text-text-muted'} ${available ? '' : 'opacity-50'}`}><input type="radio" name="image-provider" value={item.id} checked={provider === item.id} disabled={!available} onChange={() => setProvider(item.id)} /><span className="font-semibold">{item.label}</span><span className="ml-auto text-[10px]">{available ? 'disponible' : 'non configuré'}</span></label>;
              })}
            </div>
          </fieldset>
