  ...
  return (
    <article className="rounded-[11px] border border-border bg-surface p-3" data-testid="deliverable-row">
      <div className="flex items-start gap-3">
        <span className={`mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-[9px] ${status.tintClass} ${status.textClass}`}><Icon className="h-4 w-4" /></span>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          ...
          {view?.project && <>
            <section className="mt-4 rounded-[13px] border border-border bg-surface p-4">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-[9px] bg-surface-2 p-2"><div className="text-lg font-bold text-text">{detail?.deliverables.status === 'ready' ? view.deliverables.length : '—'}</div><div className="text-xs text-text-muted">Livrables</div></div><div className="rounded-[9px] bg-accent-tint p-2"><div className="text-lg font-bold text-success">{detail?.deliverables.status === 'ready' ? view.validated : '—'}</div><div className="text-xs text-text-muted">Validés</div></div><div className="rounded-[9px] bg-surface-2 p-2"><div className="text-lg font-bold text-text">{detail?.tasks.status === 'ready' ? view.tasks.length : '—'}</div><div className="text-xs text-text-muted">Tâches ouvertes</div></div></div>
