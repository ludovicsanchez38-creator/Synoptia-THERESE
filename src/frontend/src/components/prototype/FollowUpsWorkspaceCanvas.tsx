...
export function FollowUpsWorkspaceCanvas({
  ...
  return (
    <aside ref={dialogRef} role="region" aria-labelledby="follow-ups-workspace-title" tabIndex={-1} className="absolute inset-y-0 right-0 z-20 flex h-full w-full flex-col border-l border-border bg-surface-2 shadow-[-18px_0_45px_rgba(16,28,54,0.12)] sm:w-[calc(100%-48px)] xl:relative xl:w-[58%] xl:min-w-[680px] xl:shadow-none" data-testid="follow-ups-workspace-canvas">
      <header className="relative shrink-0 border-b border-border bg-surface px-5 py-4 pr-16">
        <div className="flex items-start gap-3">
          <span className="grid h-9 w-9 place-items-center rounded-md border border-text bg-[var(--color-warning-tint)] text-warning shadow-[var(--shadow-card)]"><Bell className="h-4 w-4" /></span>
        ...
        <button type="button" onClick={onClose} aria-label="Fermer les relances" className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-md border border-border bg-surface text-text-muted"><PanelRightClose className="h-4 w-4" /></button>
      </header>

      <div className="flex min-h-0 flex-1 flex-col p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div role="toolbar" aria-label="Filtrer les relances" className="flex flex-wrap gap-1.5">{FILTERS.map((entry) => <button key={entry.id} data-follow-up-filter type="button" aria-pressed={filter === entry.id} tabIndex={filter === entry.id ? 0 : -1} onKeyDown={(event) => handleRovingFocus(event, '[data-follow-up-filter]', 'horizontal')} onClick={() => setFilter(entry.id)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${filter === entry.id ? 'border-accent-fill bg-accent-fill text-accent-ink' : 'border-border bg-surface text-text-muted'}`}>{entry.label}</button>)}</div>
          <div className="flex gap-2"><button type="button" onClick={() => void refresh()} aria-label="Actualiser les relances" className="grid h-8 w-8 place-items-center rounded-sm border border-border bg-surface text-text-muted"><RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} /></button><button type="button" onClick={onOpenEmail} className="inline-flex items-center gap-1.5 rounded-sm bg-accent-fill px-3 py-2 text-xs font-semibold text-accent-ink"><Mail className="h-3.5 w-3.5" />Créer depuis un email</button></div>
        </div>

        {error && (
          <div id="follow-ups-error" role="alert" className="mt-3 flex items-center gap-2 rounded-md border border-error/40 bg-[var(--color-error-tint)] p-3 text-xs text-error">
            <AlertCircle className="h-4 w-4 shrink-0" />
            <span className="min-w-0 flex-1">{error}</span>
            <button type="button" onClick={() => void refresh()} className="min-h-11 rounded-sm border border-error px-3 font-semibold">Réessayer</button>
          </div>
        )}
