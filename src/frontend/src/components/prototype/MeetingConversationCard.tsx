    ...
    <section aria-labelledby="meeting-agenda-title" className="overflow-hidden rounded-md border border-border bg-surface shadow-[0_12px_28px_-22px_rgba(16,28,54,0.45)]" data-testid="meeting-agenda-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onNewEvent} className="inline-flex items-center gap-1.5 rounded-sm bg-accent-fill px-2.5 py-1.5 text-sm font-semibold text-accent-ink">
      ...
      {resource.status === 'loading' ? (
        ...
        <StateShell>
          <div className="max-w-sm text-center" data-testid="meeting-agenda-error">
            ...
            <div className="mt-4 flex justify-center gap-2">
              ...
              <button type="button" onClick={onRetry} className="inline-flex items-center gap-1.5 rounded-md bg-accent-fill px-3 py-2 text-sm font-semibold text-accent-ink"><RefreshCw className="h-3.5 w-3.5" />Réessayer</button>
            </div>
          </div>
        </StateShell>
      ) : events.length === 0 ? (
        <StateShell>
          <div className="text-center" data-testid="meeting-agenda-empty">
            ...
            <button type="button" onClick={onNewEvent} className="mt-4 rounded-md bg-accent-fill px-3 py-2 text-sm font-semibold text-accent-ink">Préparer un événement</button>
          </div>
        </StateShell>
      ) : (
      ...
      <div className="rounded-md border border-warning/40 bg-[var(--color-warning-tint)] p-4 text-sm text-warning" data-testid="meeting-calendar-provisioning-error">
        ...
        <div className="mt-3 flex gap-2">
          {/* Le pied de page du canevas porte déjà « Ouvrir Agenda », en
              ...
              sur le même écran. */}
          <button type="button" onClick={runEnsure} className="rounded-md bg-accent-fill px-3 py-2 text-sm font-semibold text-accent-ink">Réessayer</button>
        </div>
      </div>
    );
  }
...
function NewEventForm({
  ...
  function prepare() {
    ...
    const firstMissingId = !summary.trim() ? 'meeting-summary' : !start ? 'meeting-start' : !end ? 'meeting-end' : !calendarId ? 'meeting-calendar' : null;
    if (firstMissingId) {
      setError('Le calendrier, le titre, le début et la fin sont obligatoires.');
      setErrorFieldId(firstMissingId);
    ...
    if (new Date(end).getTime() <= new Date(start).getTime()) {
      ...
      setErrorFieldId('meeting-end');
      requestAnimationFrame(() => document.getElementById('meeting-end')?.focus());
      return;
    }
    setConfirming(true);
  }
    ...
    <div className="space-y-4" data-testid="meeting-new-event-form">
      ...
      {!confirming ? (
        ...
        <div className="rounded-md border border-accent-cyan/30 bg-accent-tint p-4">
          ...
          <div className="mt-3 flex gap-2">
            <button type="button" disabled={pending} onClick={onAbandon} className="flex-1 rounded-md border border-error bg-surface px-3 py-2 text-sm font-semibold text-error">Annuler</button>
            <button type="button" disabled={pending} onClick={() => setConfirming(false)} className="flex-1 rounded-md border border-border bg-surface px-3 py-2 text-sm font-semibold text-text">Modifier</button>
            <button type="button" disabled={pending} onClick={() => void confirm()} className="flex flex-1 items-center justify-center gap-2 rounded-md bg-accent-fill px-3 py-2 text-sm font-semibold text-accent-ink disabled:opacity-60">{pending && <Spinner taille="ligne" />}Confirmer la création</button>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="space-y-5" data-testid="meeting-event-preparation">
      ...
      {relatedContacts.length > 0 && (
        <section className="rounded-md border border-border bg-surface p-4">
          ...
          <select aria-label="Contact destinataire de la note" value={contactId} onChange={(event) => { setContactId(event.target.value); setConfirming(false); }} className="mt-3 w-full rounded-md border border-border bg-surface px-3 py-2 text-xs outline-none focus:border-accent">{relatedContacts.map((contact) => <option key={contact.id} value={contact.id}>{contactDisplayName(contact)}</option>)}</select>
          <textarea aria-label="Note de rendez-vous" value={note} onChange={(event) => { setNote(event.target.value); setConfirming(false); }} rows={4} placeholder="Tes notes factuelles après le rendez-vous…" className="mt-2 w-full resize-y rounded-md border border-border px-3 py-2 text-xs leading-5 outline-none focus:border-accent" />
          {!confirming ? (
