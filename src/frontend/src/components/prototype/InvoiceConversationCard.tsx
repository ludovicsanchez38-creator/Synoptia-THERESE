    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex gap-2">
          ...
          <BoutonOuvrirLaVue vue="invoices" onOuvrir={onOpenClassic} />
        </div>
      </div>
...
function DevisDraftForm({
  ...
  function invalidateDraft() {
    ...
    if (created) setHasUnsavedChanges(true);
    setCreated(null);
  }
  ...
  function validate(): { message: string; fieldId: string } | null {
    if (!contactId) return { message: 'Sélectionne un contact.', fieldId: 'devis-contact' };
    if (lines.length === 0) return { message: 'Ajoute au moins une ligne.', fieldId: 'devis-add-line' };
    const descriptionIndex = lines.findIndex((line) => !line.description.trim());
    if (descriptionIndex >= 0) return { message: 'Chaque ligne doit avoir une description.', fieldId: `devis-description-${descriptionIndex}` };
    const quantityIndex = lines.findIndex((line) => (parseDecimal(line.quantity) || 0) < 1);
    if (quantityIndex >= 0) return { message: 'Chaque quantité doit être supérieure ou égale à 1.', fieldId: `devis-quantity-${quantityIndex}` };
    const priceIndex = lines.findIndex((line) => parseDecimal(line.unitPrice) === null || (parseDecimal(line.unitPrice) || 0) < 0);
    if (priceIndex >= 0) return { message: 'Chaque prix HT doit être un nombre positif ou nul.', fieldId: `devis-price-${priceIndex}` };
    if (!issueDate) return { message: 'La date d’émission est obligatoire.', fieldId: 'devis-issue-date' };
    if (!dueDate || dueDate < issueDate) return { message: dueDate ? 'L’échéance ne peut pas précéder la date d’émission.' : 'L’échéance est obligatoire.', fieldId: 'devis-due-date' };
    const validityDays = Number.parseInt(validity, 10);
    if (!Number.isFinite(validityDays) || validityDays < 1) return { message: 'La durée de validité doit être positive.', fieldId: 'devis-validity' };
    return null;
  }
  ...
  async function createFirstContact() {
    ...
    if (!contactForm.firstName.trim() && !contactForm.lastName.trim() && !contactForm.company.trim()) {
      ...
      requestAnimationFrame(() => document.getElementById('devis-contact-first-name')?.focus());
      return;
    }
          ...
          <section className="rounded-md border border-border bg-surface p-4">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <label htmlFor="invoiceconversationcard-nom" className="text-xs text-text-muted">Nom
                <input id="invoiceconversationcard-nom" aria-label="Nom du nouveau contact" value={contactForm.lastName} onChange={(event) => setContactForm((current) => ({ ...current, lastName: event.target.value }))} className="mt-1.5 w-full rounded-sm border border-border bg-surface px-2.5 py-2 text-sm text-text" />
              </label>
              <label htmlFor="invoiceconversationcard-entreprise" className="text-xs text-text-muted">Entreprise
                <input id="invoiceconversationcard-entreprise" aria-label="Entreprise du nouveau contact" value={contactForm.company} onChange={(event) => setContactForm((current) => ({ ...current, company: event.target.value }))} className="mt-1.5 w-full rounded-sm border border-border bg-surface px-2.5 py-2 text-sm text-text" />
              </label>
              <label htmlFor="invoiceconversationcard-email" className="text-xs text-text-muted">Email
                <input id="invoiceconversationcard-email" aria-label="Email du nouveau contact" type="email" value={contactForm.email} onChange={(event) => setContactForm((current) => ({ ...current, email: event.target.value }))} className="mt-1.5 w-full rounded-sm border border-border bg-surface px-2.5 py-2 text-sm text-text" />
              </label>
              <label htmlFor="invoiceconversationcard-telephone" className="text-xs text-text-muted">Téléphone
                <input id="invoiceconversationcard-telephone" aria-label="Téléphone du nouveau contact" type="tel" value={contactForm.phone} onChange={(event) => setContactForm((current) => ({ ...current, phone: event.target.value }))} placeholder="06 12 34 56 78" className="mt-1.5 w-full rounded-sm border border-border bg-surface px-2.5 py-2 text-sm text-text" />
              </label>
              <label htmlFor="invoiceconversationcard-adresse" className="text-xs text-text-muted sm:col-span-2">Adresse
                <input id="invoiceconversationcard-adresse" aria-label="Adresse du nouveau contact" value={contactForm.address} onChange={(event) => setContactForm((current) => ({ ...current, address: event.target.value }))} placeholder="14 chemin des Oliviers, 04300 Mane" className="mt-1.5 w-full rounded-sm border border-border bg-surface px-2.5 py-2 text-sm text-text" />
              </label>
            </div>
