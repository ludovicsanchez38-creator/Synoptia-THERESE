...
function SynthesisView({ synthesis }: { synthesis: BoardSynthesis }) {
  return (
    <div className="space-y-3" data-testid="board-synthesis">
      ...
      <section className="rounded-md border border-border bg-surface p-4"><h4 className="text-xs font-bold text-text">Prochaines étapes</h4><ol className="mt-2 space-y-2 text-xs leading-5 text-text">{synthesis.next_steps.map((step, index) => <li key={step} className="flex gap-2"><span className="font-bold text-[var(--k4)]">{index + 1}</span>{step}</li>)}</ol></section>
    </div>
  );
}
...
function AdvisorOpinionCard({ advisor, info }: { advisor?: PrototypeAdvisorState; info: AdvisorInfo }) {
  return (
    <section className="rounded-md border border-border bg-surface p-3">
      <div className="flex items-center gap-2.5"><CharacterPortrait index={advisorPortraits[info.role]} className="h-8 w-8 rounded-sm border border-text" /><div className="min-w-0 flex-1"><h4 className="text-xs font-bold text-text">{info.name}</h4><p className="truncate text-xs text-text-muted">{advisor?.provider || info.personality}</p></div>{advisor?.isRunning ? <Spinner taille="ligne" className="text-[var(--k4)]" /> : advisor?.isComplete ? <CheckCircle2 className="h-3.5 w-3.5 text-success" /> : <span className="h-2 w-2 rounded-full bg-border" />}</div>
...
function NewBoardForm({ advisors, run, onStart }: { advisors: AdvisorInfo[]; run: BoardRunState; onStart: (request: BoardRequest) => Promise<void> }) {
  ...
  return (
    <div className="space-y-4" data-testid="board-new-form">
      ...
      <section className="rounded-md border border-border bg-surface p-4"><label className="block text-sm font-semibold text-text">Question stratégique<textarea id="board-question" aria-label="Question stratégique" aria-invalid={Boolean(error)} aria-describedby={error ? 'board-form-error' : undefined} value={question} onChange={(event) => { setQuestion(event.target.value); setError(null); }} placeholder="Quelle décision veux-tu éclairer ?" className="mt-2 h-28 w-full resize-y rounded-md border border-border p-3 text-sm font-normal leading-6 text-text outline-none focus:border-[var(--k4)]" /></label><label className="mt-3 block text-sm font-semibold text-text">Contexte utile, facultatif<textarea aria-label="Contexte du Board" value={context} onChange={(event) => setContext(event.target.value)} placeholder="Contraintes, hypothèses, chiffres ou échéance…" className="mt-2 h-24 w-full resize-y rounded-md border border-border p-3 text-sm font-normal leading-5 text-text outline-none focus:border-[var(--k4)]" /></label></section>
      <section className="rounded-md border border-border bg-surface p-4"><h3 id="board-mode-label" className="text-sm font-bold text-text">Mode de délibération</h3><div className="mt-3 grid grid-cols-2 gap-2" role="radiogroup" aria-labelledby="board-mode-label"><button type="button" role="radio" aria-checked={mode === 'cloud'} tabIndex={mode === 'cloud' ? 0 : -1} onKeyDown={(event) => handleRovingFocus(event, '[role="radio"]', 'horizontal')} onClick={() => setMode('cloud')} className={`rounded-md border p-3 text-left ${mode === 'cloud' ? 'border-[var(--k4)] bg-[var(--k4bg)]' : 'border-border'}`}><Globe className="h-4 w-4 text-[var(--k4)]" /><strong className="mt-2 block text-sm text-text">Cloud</strong><span className="mt-1 block text-sm leading-4 text-text-muted">Providers configurés et recherche web.</span></button><button type="button" role="radio" aria-checked={mode === 'sovereign'} tabIndex={mode === 'sovereign' ? 0 : -1} onKeyDown={(event) => handleRovingFocus(event, '[role="radio"]', 'horizontal')} onClick={() => setMode('sovereign')} className={`rounded-md border p-3 text-left ${mode === 'sovereign' ? 'border-[var(--k4)] bg-[var(--k4bg)]' : 'border-border'}`}><ShieldCheck className="h-4 w-4 text-[var(--k4)]" /><strong className="mt-2 block text-sm text-text">Souverain</strong><span className="mt-1 block text-sm leading-4 text-text-muted">Ollama local, sans recherche web.</span></button></div></section>
      <section className="rounded-md border border-border bg-surface p-4"><div className="flex items-center gap-2 text-xs font-bold text-text"><Users className="h-4 w-4 text-[var(--k4)]" />Conseillers réellement configurés</div><div className="mt-3 grid grid-cols-5 gap-2">{advisorOrder.map((role) => { const advisor = advisors.find((item) => item.role === role); return <div key={role} className="text-center"><CharacterPortrait index={advisorPortraits[role]} className="mx-auto h-9 w-9 rounded-sm border border-text" /><span className="mt-1 block truncate text-xs text-text-muted">{advisor?.name || role}</span></div>; })}</div></section>
      </fieldset>
