    ...
    <section aria-labelledby="board-history-title" className="overflow-hidden rounded-[16px] border border-border bg-surface shadow-[0_12px_28px_-22px_rgba(16,28,54,0.45)]" data-testid="board-history-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onNewBoard} className="inline-flex items-center gap-1.5 rounded-[8px] bg-accent-fill px-2.5 py-1.5 text-xs font-semibold text-accent-ink"><Plus className="h-3.5 w-3.5" />Nouvelle question</button>
      ...
      {resource.status === 'loading' ? (
        ...
        <StateShell><div className="text-center" data-testid="board-history-empty"><Gavel className="mx-auto h-6 w-6 text-text-muted" /><p className="mt-2 text-sm font-semibold text-text">Aucune décision enregistrée</p><p className="mt-1 text-xs text-text-muted">Une délibération ne démarrera qu’après ta confirmation.</p><button type="button" onClick={onNewBoard} className="mt-4 rounded-[9px] bg-accent-fill px-3 py-2 text-xs font-semibold text-accent-ink">Convoquer le Board</button></div></StateShell>
      ) : (
...
function NewBoardForm({ advisors, run, onStart }: { advisors: AdvisorInfo[]; run: BoardRunState; onStart: (request: BoardRequest) => Promise<void> }) {
  ...
  return (
    <div className="space-y-4" data-testid="board-new-form">
      ...
      <section className="rounded-[13px] border border-border bg-surface p-4"><label className="block text-sm font-semibold text-text">Question stratégique<textarea id="board-question" aria-label="Question stratégique" aria-invalid={Boolean(error)} aria-describedby={error ? 'board-form-error' : undefined} value={question} onChange={(event) => { setQuestion(event.target.value); setError(null); }} placeholder="Quelle décision veux-tu éclairer ?" className="mt-2 h-28 w-full resize-y rounded-[9px] border border-border p-3 text-sm font-normal leading-6 text-text outline-none focus:border-[var(--k4)]" /></label><label className="mt-3 block text-sm font-semibold text-text">Contexte utile, facultatif<textarea aria-label="Contexte du Board" value={context} onChange={(event) => setContext(event.target.value)} placeholder="Contraintes, hypothèses, chiffres ou échéance…" className="mt-2 h-24 w-full resize-y rounded-[9px] border border-border p-3 text-sm font-normal leading-5 text-text outline-none focus:border-[var(--k4)]" /></label></section>
      <section className="rounded-[13px] border border-border bg-surface p-4"><h3 id="board-mode-label" className="text-xs font-bold text-text">Mode de délibération</h3><div className="mt-3 grid grid-cols-2 gap-2" role="radiogroup" aria-labelledby="board-mode-label"><button type="button" role="radio" aria-checked={mode === 'cloud'} tabIndex={mode === 'cloud' ? 0 : -1} onKeyDown={(event) => handleRovingFocus(event, '[role="radio"]', 'horizontal')} onClick={() => setMode('cloud')} className={`rounded-[10px] border p-3 text-left ${mode === 'cloud' ? 'border-[var(--k4)] bg-[var(--k4bg)]' : 'border-border'}`}><Globe className="h-4 w-4 text-[var(--k4)]" /><strong className="mt-2 block text-xs text-text">Cloud</strong><span className="mt-1 block text-xs leading-4 text-text-muted">Providers configurés et recherche web.</span></button><button type="button" role="radio" aria-checked={mode === 'sovereign'} tabIndex={mode === 'sovereign' ? 0 : -1} onKeyDown={(event) => handleRovingFocus(event, '[role="radio"]', 'horizontal')} onClick={() => setMode('sovereign')} className={`rounded-[10px] border p-3 text-left ${mode === 'sovereign' ? 'border-[var(--k4)] bg-[var(--k4bg)]' : 'border-border'}`}><ShieldCheck className="h-4 w-4 text-[var(--k4)]" /><strong className="mt-2 block text-xs text-text">Souverain</strong><span className="mt-1 block text-xs leading-4 text-text-muted">Ollama local, sans recherche web.</span></button></div></section>
