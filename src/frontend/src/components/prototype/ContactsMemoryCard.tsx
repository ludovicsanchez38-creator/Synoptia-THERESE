...
function ContactAvatar({ contact, className = '' }: { contact: Contact; className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={`grid shrink-0 place-items-center rounded-[10px] border border-[#CFE9EE] bg-[#E8F8FB] font-bold text-[#0B7896] ${className}`}
    >
  ...
  return (
    <div className="flex min-h-44 items-center justify-center px-5 py-8 text-center" data-testid="contacts-memory-empty">
      <div>
        ...
        <p className="mt-2 text-sm font-semibold text-[#101C36]">Aucun contact enregistré</p>
        <p className="mt-1 text-xs text-[#5B6A82]">La mémoire est prête, mais elle ne contient encore aucune personne.</p>
        <button
          type="button"
          onClick={onOpenClassic}
          className="mt-4 rounded-[9px] border border-[#101C36] bg-[#101C36] px-3 py-2 text-xs font-semibold text-white"
        >
          Ouvrir la mémoire
        </button>
      </div>
    ...
    >
      <div className="flex items-center justify-between border-b border-[#E7ECF4] px-4 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 id="contacts-memory-title" className="text-sm font-semibold text-[#101C36]">Contacts et mémoire</h2>
      ...
      {resource.status === 'loading' ? (
        <div className="flex min-h-44 items-center justify-center gap-2 px-5 py-8 text-sm text-[#5B6A82]" role="status">
        ...
        <div className="flex min-h-44 items-center justify-center px-5 py-8 text-center" data-testid="contacts-memory-error">
          <div>
            ...
            <p className="mt-2 text-sm font-semibold text-[#101C36]">Mémoire indisponible</p>
            <p className="mt-1 text-xs text-[#5B6A82]">{resource.error}</p>
            <button
              type="button"
              onClick={onRetry}
              className="mt-4 inline-flex items-center gap-1.5 rounded-[9px] border border-[#101C36] bg-[#101C36] px-3 py-2 text-xs font-semibold text-white"
            >
          ...
          {recentContacts.map((contact) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-semibold text-[#101C36]">{contactDisplayName(contact)}</span>
                <span className="mt-0.5 block truncate text-xs text-[#5B6A82]">
      ...
      {resource.status === 'ready' && resource.data.length > recentContacts.length && (
        <button
          type="button"
          onClick={onOpenClassic}
          className="w-full border-t border-[#E7ECF4] bg-[#F8FAFD] px-4 py-3 text-center text-xs font-semibold text-[#6D4CC3] hover:bg-[#F3F0FB]"
        >
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-[#E4EAF3] px-5 py-4 pr-16">
        ...
        <h2 className="mt-2 text-xl font-bold tracking-[-0.02em] text-[#101C36]">Contacts et contexte</h2>
        <p className="mt-1 text-sm text-[#5B6A82]">Lecture seule des fiches réellement enregistrées dans Thérèse.</p>
      </div>

      {resource.status === 'loading' ? (
        <div className="flex flex-1 items-center justify-center gap-2 text-sm text-[#5B6A82]">
          <Loader2 className="h-4 w-4 animate-spin text-[#7C3AED]" />
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[210px_minmax(0,1fr)]">
          <aside className="min-h-0 border-r border-[#E4EAF3] bg-white p-3">
            <div className="relative mb-3">
              <Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-[#7B8AA3]" />
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                aria-label="Rechercher un contact"
                placeholder="Rechercher…"
                className="w-full rounded-[9px] border border-[#DCE4F1] bg-[#F7F9FD] py-2 pl-8 pr-2 text-xs text-[#101C36] outline-none focus:border-[#7C3AED]"
              />
            </div>
              ...
              {filteredContacts.map((contact) => (
                <button
                  ...
                  className={`flex w-full items-center gap-2 rounded-[9px] px-2 py-2 text-left ${
                    selectedContact?.id === contact.id ? 'bg-[#F0E9FC]' : 'hover:bg-[#F7F9FD]'
                  }`}
                >
                  <ContactAvatar contact={contact} className="h-7 w-7 text-[9px]" />
                  <span className="min-w-0 flex-1 truncate text-xs font-semibold text-[#33415C]">{contactDisplayName(contact)}</span>
                </button>
              ))}
              {filteredContacts.length === 0 && (
                <p className="px-2 py-4 text-center text-xs text-[#7B8AA3]">Aucun contact trouvé.</p>
              )}
            </div>
          </aside>
              ...
              <div>
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-lg font-bold text-[#101C36]">{contactDisplayName(selectedContact)}</h3>
                    <p className="mt-0.5 text-xs text-[#5B6A82]">
                ...
                <div className="mt-5 grid gap-2">
                  {selectedContact.email && (
                    <div className="flex items-center gap-2 rounded-[10px] border border-[#E4EAF3] bg-white px-3 py-2.5 text-xs text-[#33415C]">
                      <Mail className="h-3.5 w-3.5 text-[#7C3AED]" />
                      {selectedContact.email}
                    </div>
                  )}
                  {selectedContact.phone && (
                    <div className="rounded-[10px] border border-[#E4EAF3] bg-white px-3 py-2.5 text-xs text-[#33415C]">
                      {selectedContact.phone}
                    </div>
                  )}
                </div>
                ...
                <div className="mt-5">
                  <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#7B8AA3]">Notes mémorisées</div>
                  <div className="mt-2 min-h-24 whitespace-pre-wrap rounded-[12px] border border-[#E4EAF3] bg-white p-3 text-sm leading-6 text-[#33415C]">
                ...
                {selectedContact.tags && selectedContact.tags.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {selectedContact.tags.map((tag) => (
                      <span key={tag} className="rounded-full bg-[#F0E9FC] px-2 py-1 text-[10px] font-semibold text-[#6D4CC3]">{tag}</span>
                    ))}
                  </div>
                )}
              </div>
