...
> **v0.32.0-alpha RELEASÉE le 11/07/2026 midi** : Variables V1 complètes (co-design Dr_logic : texte|liste, verbes chat zéro-LLM `{action: variable ...}`, substitution `{nom}` single-pass bornée, preview, rubrique Réglages > Services, chip d'aperçu) + durcissement sécurité pipeline chat (tranche 0 design V4, NO-GO Codex, findings VÉRIFIÉS : ReDoS parser, produire immuable, safety sur payload réel, /fichier sur brut, history sans échanges déterministes). **Corrigé sur main POST-tag (=> 0.32.1, `d27395f`)** : BUG-122 Favoris IMAP par flag \Flagged, BUG-136 une carte par fichier (collecteur ContextVar + `extra_data.skill_files` liste, lecture legacy `skill_file` conservée), BUG-137 détection d'intention par mots-clés SUPPRIMÉE (effet de bord = demande explicite). Chantier 3 Ollama auto-hébergé DIFFÉRÉ (Ludo). Rapport docs/releases/v0.32.0-alpha.md.
