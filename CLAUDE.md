...
### Depuis fixes P1 rapport Syn 14/06/2026 (commits 36922e9..62a4d8c, sur main, PAS encore releasé)
12 P1 backend du rapport de tests Syn corrigés (chacun = fix + test de régression). À embarquer dans la prochaine release via /release-therese. Reste à faire et dette résiduelle :
- [ ] **`/api/shutdown` sans token (CSRF)** : NON corrigé ici car nécessite un changement Rust (`lib.rs`) + rebuild Tauri signé par les 2 appelants (token `~/.therese/.session_token`). À traiter dans /release-therese. Risque accepté en l'état (ferme l'app, récupérable ; atténué par Private Network Access navigateur).
- [ ] **Usage tokens estimé, pas réel** : chat non-stream ET board estiment `~2 tokens/mot` (`len(text.split())*2`) au lieu de lire l'usage réel des providers (`usage.input_tokens` Anthropic, `usageMetadata.promptTokenCount` Gemini, etc.). Suffisant pour le budget/garde-fou, mais imprécis. Propager l'usage réel = refactor providers (chaque `stream_response_with_tools` devrait émettre un StreamEvent de métriques en fin de tour).
