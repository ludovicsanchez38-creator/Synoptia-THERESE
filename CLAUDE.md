...
> **v0.48.0-alpha RELEASÉE le 25/08/2026 (soir)** - jalon « un établi, un tiroir » (21 commits, rapport `docs/releases/v0.48.0-alpha.md`). Board en frontier+effort max (catalogue neutre `modeles_catalogue.py` source unique, effort résolu par construction de LLMConfig, 4 tables llm.py dérivées, transport du brut Mistral reasoning, repli Grok 400, `bascule_circuit=False` + `raise_on_error=True` aux 3 stream_response du Board, plancher 16000 tokens de sortie), manifeste étendu au tiroir (binding `{registre:"tiroir",carte}`), lexique verrouillé par test sur les registres (Travaux/Décision/Contacts/Pipeline/Agenda/Paramètres... - table `docs/rules/RULES-DESIGN.md` §13) + frontière d'erreurs (`message_pour_ecran`/`ErreurPourEcran`, forme CLASSÉE des providers réseau/interne pour le circuit breaker), établi B1 (Écrire/Retrouver/Préparer/Facturer, porte « Plus d'outils »), sonde de dérive (cloud only, 1×/jour). Prix frontiers relevés aux sources 25/08 dans TOKEN_PRICES. Revue : design 7 passes + 4 vagues implémentation (31 findings fermés) ; **passe Soso finale NON tenue (crédits Codex épuisés)** → à faire en contrôle post-release (cibles listées dans le rapport). Dette : trou jumeau comptabilité circuit breaker chat stream (préexistant), bascule cloud chez souverain à clé stockée (préexistant), validation réelle Mistral reasoning.

> **v0.45.0-alpha RELEASÉE le 24/08/2026** (3 jalons : manifeste 0.44, dette 0.43.4, project.sync 0.45 - 11 passes Soso cumulées sur la journée). Dette actée et backlog Windows (11 tests rouges sur windows-latest, job hors gate) : voir `docs/releases/v0.45.0-alpha.md`. **Gotcha release** : les POST Discord depuis le VPS exigent désormais `User-Agent: DiscordBot (...)` (403 Cloudflare `error code: 1010` sinon). **Prochain gros chantier recommandé** : registre de tâches/annulation (suggestions 1-2-12), puis cycle documentaire.

> **v0.43.4-alpha RELEASÉE le 24/08/2026** : lot « confiance au premier usage », issu des 7 fiches d'un nouveau testeur (Jérôme, AMD E1-7010 1,5 GHz - la machine la plus modeste vue en alpha, révélatrice de tout ce que l'app suppose d'un poste rapide). **BUG-172** faux timeout profil (profil écrit puis embedding 19 s AVANT la réponse ; indexation passée en tâche de fond avec numéro de génération - un verrou ne garantit pas l'ordre, une annulation ne marche pas car `to_thread` n'est pas annulable et pouvait EFFACER le profil), **BUG-171** faux brouillon email (le générateur fabriquait « Je reviens vers vous rapidement » en HTTP 200 ; remplacé par `GenerationImpossible` + `cause_lisible()` + 502, les DEUX écrans affichent la cause), **BUG-169** modèles Ollama sans étiquette « tools » écartés du sélecteur et grisés avec motif à l'onboarding (`ollama_capabilites.py`, doute au bénéfice du modèle inconnu), **BUG-166/167** revérification Ollama + promesse du dossier corrigée, logs Windows en UTF-8. **Sécurité** : `_mask_secrets` ne masquait AUCUNE clé moderne (`sk-proj-` refusé par le motif) - corrigé avec frontière gauche (sans elle « task-scheduler » devenait « ta***MASKED*** ») + trace d'exception masquée dans les DEUX formatters. **Catalogue modèles** relevé en doc officielle le 24/08 (Opus 5/Fable 5/Sonnet 5, GPT-5.6 sol/terra/luna, Grok 4.6, Gemini 3.7 Flash, DeepSeek v4 - `deepseek-chat`/`reasoner` ONT DISPARU de l'API) + 4 fournisseurs OpenAI-compatibles GLM/Kimi/Qwen/MiniMax (héritent d'OpenAIProvider ; PIÈGE : les Literal Pydantic ET le type frontend devaient les accepter, sinon fournisseur déclaré mais inconfigurable). `gpt-5.3-codex` RETIRÉ : sa fiche officielle dit « Chat Completions Not supported » (contesté par la revue, vérifié à la source, retrait maintenu). Rituel : 2 passes Soso, 2 NO-GO (8 + 7 findings), 15 défauts fermés dont 2 régressions que la remédiation avait elle-même introduites. **Dette assumée** : Qwen déclaré mais pas configurable de bout en bout (`base_url` avec `{EspaceDeTravail}` sans champ de config, `url_effective()` jamais appelée par `OpenAIProvider`) ; les 4 fournisseurs absents des listes de l'onboarding et du runtime agents ; catalogue de modèles encore dupliqué à 4 endroits (backend, Atelier, LLMTab, manifeste) - c'est le chantier 0.44 ; aucune CI Windows alors que tous les rapports viennent de Windows ; `test_produire_docx_stream_fichier_reel` échoue au teardown sur main (préexistant). Rapport `docs/releases/v0.43.4-alpha.md`.


> **v0.41.3-alpha RELEASÉE le 27/07/2026** : lot du triage Discord 22-27/07 (BUG-154 à 159). Le sujet central est **BUG-155** : l'indexation gelait TOUTE l'application. `extract_text`/`chunk_text` sont synchrones et étaient appelés depuis des routes `async` - la boucle d'événements restait bloquée, plus aucune requête n'était servie. Corrigé sur **quatre chemins** : `/api/files/index`, `/api/files/upload` (pièce jointe de projet), `/api/files/{id}/content`, et `chat.py::_get_file_context` (fichier joint à un message - trouvé en contre-vérifiant la revue). S'y ajoute un défaut plus grave que le gel : la transaction SQLite restait ouverte pendant tout le traitement, et SQLite étant mono-écrivain (`busy_timeout` 5 s), une écriture concurrente pouvait **échouer**. `index_payload` encadre désormais le travail lourd de deux transactions courtes, avec verrou par chemin (`path` est UNIQUE), sémaphore à 2 places sur les encodages, et abandon consulté avant chaque étape coûteuse. **Invariant à ne jamais casser** : les anciens vecteurs ne sont supprimés qu'au moment d'écrire les nouveaux ; si l'écriture échoue, la base consigne `chunk_count=0` plutôt que de promettre un contenu introuvable. Autres fiches : BUG-154 (badge « En cours » qui désignait l'état de rédaction, pas un traitement -> Sans trame/Rédaction/Terminé + trame générée à la création), BUG-156 (Paramètres fermés au clic sur le fond), BUG-157/159 (bouton Paramètres permanent, état de chargement du profil, rubriques masquées nommées), BUG-158, messages du triage. Rituel Soso : **3 passes, 2 NO-GO** (8 findings puis 5 défauts nouveaux, tous vérifiés puis fermés). Rapport `docs/releases/v0.41.3-alpha.md`. **Dette assumée** : une extraction déjà lancée va à son terme (`run_in_threadpool` est en `abandon_on_cancel=False`) ; `upload_file` garde une transaction longue (préexistant). **Reste** : BUG-140 (lcjp), BUG-142, BUG-146 volet corruption, dette F5 (tri emails sur header `Date` forgeable -> `internalDate` Gmail), retrait du mode classic en 0.42.

> **v0.41.1-alpha RELEASÉE le 20/07/2026** : lot des 8 fiches Dr_logic 0.41.0 Win10 (triage 18/07). BUG-143 (provisionnement calendrier local au geste de création - la coque lit en createDefault:false), BUG-147 (update lignes facture : collection détachée avant delete, expire_on_commit=False -> réaffecter les nouvelles lignes), BUG-141 (BROWSER_TOOL annoncé seulement si playwright importable - extra e2e jamais packagé), BUG-139 suite (commandes / de navigation = exécution directe via runNavigationAction, garde streaming sur état vivant), BUG-144 (début=fin accepté en toute la journée ; fin INCLUSIVE partout, conversions ±1 jour aux frontières Google/CalDAV/ICS + **migration ad-hoc unique** du cache distant marqueur preferences.allday_end_semantics), BUG-148 (outil search_invoices lecture seule, ILIKE échappé, sortie neutralisée sanitize_for_context - les marqueurs forgés [Source:/[End dans les données sont neutralisés pour TOUS les consommateurs), BUG-145 (messages micro actionnables, mapping sur texte y compris chaînes plugin Tauri, garde anti-faux diagnostic fichier). Rituel : revue Soso NO-GO 11 findings -> contre-vérif -> remédiation 2 vagues -> tous fermés (F3 dette préexistante rendu multi-jour, F11 dette dev chromium). Rapport docs/releases/v0.41.1-alpha.md. **Reste** : BUG-146 (réponses Dr_logic attendues), volet 2 BUG-148 (pièce jointe : attachments ignorés par provider Gmail), flake d'ordre web_search+chat (préexistant), retrait mode classic 0.42.

> **v0.41.0-alpha RELEASÉE le 17/07/2026 (soir)** : chantier harmonisation ancienne/nouvelle interface (14 commits). Scroll mail vue complète (wrapper flex-col, profite aux 10 vues embarquées), BUG-122 définitif (dossier IMAP non résolu = liste vide + warning, jamais l'INBOX ; sélection purgée), BUG-139 définitif (navigation chat : événement `therese:client-action` revendiqué par la coque, exécution différée dans le finally, annulée sur AbortError/message en file, garde sur l'état vivant), calendrier en dates CIVILES (`lib/civilDate.ts` localDateKey, 9 toISOString remplacés - décalage d'un jour en UTC+), board complet aux portraits officiels (BoardDialogShell + useIsPresent : le focus trap tient pendant l'exit AnimatePresence), contacts harmonisés (text-left, RGPD ?, entête responsive), avatar unifié atlas index 0 (PNG supprimé), vestiges (DataTab, advisors-bg.png). Revue Soso unique en fin de chantier (règle Ludo) : NO-GO 6 findings -> tous fermés. Mode classic INTACT (secours, retrait envisagé 0.42). Rapport docs/releases/v0.41.0-alpha.md. **Reste** : boutons pilule cyan de Facturation, retrait du mode classic, BUG-140 à confirmer avec lcjp.

> **v0.40.1-alpha RELEASÉE le 17/07/2026** : hotfix de la revue Codex 0.40 (7 anomalies vérifiées puis corrigées : E2E isolés sur backend jetable 17393 + garde anti-17293, WindowControls par OS via `lib/platform.ts`, delete backups `.enc`, pre_restore -> sauvegarde chiffrée visible avec rétention 1, delete-all purge disque images/outputs/projects + message honnête, consentement cloud v2 par finalité+fournisseur (dictée Groq au clic micro, Board = accord dédié `llm:board`, migration v1 ABANDONNÉE), sidecar auto-relancé 3x + bandeau + `restart_backend`, timeout apiFetch 30 s « attente de réponse » avec ~20 exemptions, release.yml gated sur la CI via workflow_call, rapports Playwright dé-versionnés, `.session_token` et `files.py` sur THERESE_DATA_DIR). Revue adversariale Soso NO-GO 11 findings -> tous fermés (rapport docs/releases/v0.40.1-alpha.md). **Dette assumée** : `safety_password` distinct pour l'archive pre_restore (elle est chiffrée avec la passphrase de la sauvegarde RESTAURÉE, dit à l'utilisateur - décision produit à prendre) ; `logging_config.py` + `llm.py` (THERESE.md) encore sur `~/.therese` en dur. **À suivre** : BUG-140 updater Windows (lcjp, cause plausible corrigée par F8 - à confirmer sur 0.40.1), BUG-122 rouvert (rubriques IMAP hors Favoris), BUG-139 navigation vocale, chantier harmonisation ancienne/nouvelle interface demandé par Ludo (mail complet, contacts, board complet avec avatars, scroll dans un mail ancienne interface).

> **v0.32.0-alpha RELEASÉE le 11/07/2026 midi** : Variables V1 complètes (co-design Dr_logic : texte|liste, verbes chat zéro-LLM `{action: variable ...}`, substitution `{nom}` single-pass bornée, preview, rubrique Réglages > Services, chip d'aperçu) + durcissement sécurité pipeline chat (tranche 0 design V4, NO-GO Codex, findings VÉRIFIÉS : ReDoS parser, produire immuable, safety sur payload réel, /fichier sur brut, history sans échanges déterministes). **v0.32.1-alpha RELEASÉE le 11/07 après-midi** (`d27395f`, rapport docs/releases/v0.32.1-alpha.md) : BUG-122 Favoris IMAP par flag \Flagged, BUG-136 une carte par fichier (collecteur ContextVar + `extra_data.skill_files` liste, lecture legacy `skill_file` conservée), BUG-137 détection d'intention par mots-clés SUPPRIMÉE (effet de bord = demande explicite). Chantier 3 Ollama auto-hébergé DIFFÉRÉ (Ludo). Rapport docs/releases/v0.32.0-alpha.md.

> **v0.31.0-alpha RELEASÉE le 11/07/2026 (fast-track)** : effort de raisonnement configurable (Paramètres > IA : Auto/Faible/Moyen/Élevé/Maximal, préférence `llm_effort`, champ `effort` sur GET/POST /api/config/llm). Envoyé UNIQUEMENT aux modèles au support vérifié : Anthropic `output_config.effort` (Fable 5/Sonnet 5/Sonnet 4.6/Opus 4.5+, pas Haiku), OpenAI `reasoning_effort` (GPT-5.6 seulement, 5.5/5.4 non sourcés = dette), Grok 4.5 (max plafonné high), Ollama `think` (max plafonné high + retry silencieux sans think pour les modèles non-thinking, sentinel `_THINK_UNSUPPORTED`). 22 tests `test_provider_effort.py`. RESTE : recette visuelle du sélecteur en app packagée, chantiers 4 Variables et 3 Ollama auto-hébergé (designs V2 prêts). Rapport docs/releases/v0.31.0-alpha.md.
